...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate SAM-HQ mask refinement.")
    ...
    parser.add_argument("--output", default="outputs/samhq_validation_metrics.json")
...
def mask_iou(pred_mask: np.ndarray, gt_mask: np.ndarray) -> float:
    ...
    if union == 0:
        return 0.0
    intersection = np.logical_and(pred_mask.astype(bool), gt_mask.astype(bool)).sum()
...
def load_samhq_predictor(
    ...
    try:
        ...
        raise ImportError("SAM-HQ is not installed in this environment.") from error
...
def refine_masks(
    ...
    for box in detector_boxes:
        ...
        refined_masks.append(np.asarray(masks[0], dtype=bool))
    return np.asarray(refined_masks, dtype=bool)
...
def main() -> None:
    ...
    detector = load_detector(config, args.detector_checkpoint, device)
    ...
    for index in range(max_images):
        ...
        prediction = detector([image_tensor.to(device)])[0]
        ...
        for gt_mask, gt_label in zip(gt_masks, gt_labels):
            detector_ious.append(
                best_iou_for_gt(gt_mask, gt_label, pred_masks, pred_labels)
            )
            samhq_ious.append(
                best_iou_for_gt(gt_mask, gt_label, refined_masks, pred_labels)
            )
        ...
        if args.vis_dir and visualized < args.vis_count:
            draw_visualization(
                ...
                scores,
                categories,
                Path(args.vis_dir) / f"{annotation_path.stem}_samhq.jpg",
            )
            visualized += 1
    ...
    metrics = {
        ...
        "gt_instances": len(samhq_ious),
        "detector_mean_best_mask_iou": mean(detector_ious) if detector_ious else 0.0,
        "samhq_mean_best_mask_iou": mean(samhq_ious) if samhq_ious else 0.0,
        "detector_recall_iou_50": (
            mean([iou >= 0.5 for iou in detector_ious]) if detector_ious else 0.0
        ),
        "samhq_recall_iou_50": (
            mean([iou >= 0.5 for iou in samhq_ious]) if samhq_ious else 0.0
        ),
        "detector_recall_iou_75": (
            mean([iou >= 0.75 for iou in detector_ious]) if detector_ious else 0.0
        ),
        "samhq_recall_iou_75": (
            mean([iou >= 0.75 for iou in samhq_ious]) if samhq_ious else 0.0
        ),
